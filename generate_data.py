"""
This script automates the generation of synthetic testing data for an AI-powered
...
def save_as_pdf(folder, filename, text):
    ...
    html_content = (
        f"<p style='font-family:sans-serif; font-size:11pt; text-align:justify;'>"
...
if __name__ == "__main__":
    ...
    save_as_pdf("data/rules", "educational_compliance_rules", rules_content)
    ...
    report_scenarios = [
        ...
        {"desc": "A faulty report where the Contract ID is missing from the Identity section.", "label": "fail_incomplete_id"},
        {"desc": "A perfect report satisfying all rules.", "label": "pass"},
    ]
    ...
    for i, scenario in enumerate(report_scenarios):
        ...
        STRICT RULES:
        1. If the SCENARIO says the report is 'faulty' or 'missing' a section, you MUST omit that section or field.
        2. Maintain a professional, critical, and academic tone.
        3. Keep the total length under 300 words.
        """

        content = generate_text(prompt)
        save_as_pdf("data/reports", f"report_{i + 1}_{scenario['label']}", content)
